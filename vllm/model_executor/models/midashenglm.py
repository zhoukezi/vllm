...
class DashengAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None):
        ...
        if self.causal:
            ...
            i, j = attn.shape[-2:]
            mask = torch.ones(i, j, device=q.device, dtype=torch.bool).triu(
                j - i + 1
            )
...
class DashengFrontend(nn.Module):
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        mel_spectrogram = (
            spectrogram.mT @ self.melscale_fbanks.to(torch.float32)
        ).mT
...
class DashengAudioTransformer(nn.Module):
    ...
    def forward(
        ...
        if x_length is not None:
            ...
            split_masks = mask.logical_not().split(
                target_length_in_patches, dim=-1
            )
        else:
            mask = None
    ...
    def get_dummy_mm_data(
        ...
        return {
            "audio": self._get_dummy_audios(
                length=self.info.get_max_audio_len(), num_audios=num_audios
            )
        }
...
class MiDashengLMModel(nn.Module, SupportsMultiModal, SupportsPP):
        ...
        if name == "input_values":
            ...
            padded_mm_input = [
                torch.nn.functional.pad(
                    tensor, (0, max_length - tensor.shape[1])
                )
    ...
    def _process_audio_input(
        ...
        encoder_out, encoder_atts = self.audio_encoder(
